...
if (!dir.exists("tables/Wimalasiri_DE")) {
    dir.create("tables/Wimalasiri_DE", recursive = TRUE)
}

write.csv(temp.table, "tables/Wimalasiri_DE/W.DEtemp.csv", quote = F, row.names = F)
write.csv(inf.table, 'tables/Wimalasiri_DE/w.DEinf.csv', quote=F, row.names = F)
write.csv(time.table, 'tables/Wimalasiri_DE/w.DEtime.csv', quote=F, row.names = F)
write.csv(temp_inf.table, 'tables/Wimalasiri_DE/w.DEtemp_inf.csv', quote=F, row.names = F)
write.csv(temp_time.table, 'tables/Wimalasiri_DE/w.DEtemp_time.csv', quote=F, row.names = F)
write.csv(inf_time.table, 'tables/Wimalasiri_DE/w.DEinf_time.csv', quote=F, row.names = F)
write.csv(temp_inf_time.table, 'tables/Wimalasiri_DE/w.DEtemp_inf_time.csv', quote=F, row.names = F)
...
namedf<-x[,1:5]
temp.table<-merge(temp.table, namedf, by='Gene.ID')
write.csv(temp.table, "tables/Wimalasiri_DE/Wnew.DEtemp.csv", quote = F, row.names = F)
